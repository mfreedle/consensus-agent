    ...
    def process_natural_language_request(self, message: str) -> str:
        ...
        for pattern in doc_create_patterns:
            if re.search(pattern, message_lower):
                ...
                title_match = re.search(
                    r'(?:called|titled|named)\s+"([^"]+)"', message, re.IGNORECASE
                )
                if not title_match:
                    title_match = re.search(
                        r"(?:called|titled|named)\s+([^.!?]+)", message, re.IGNORECASE
        ...
        for pattern in sheet_create_patterns:
            if re.search(pattern, message_lower):
                ...
                title = (
                    title_match.group(1).strip() if title_match else "New Spreadsheet"
                )
    ...
    def show_my_drive_files(self, max_results: int = 10) -> str:
        ...
        try:
            ...
            if result.startswith("[") or result.startswith("{"):
                ...
                response = (
                    f"📁 **Your Google Drive Files** (showing {len(files)} files):\n\n"
                )
                for i, file in enumerate(files, 1):
                    ...
                    modified = (
                        file.get("modifiedTime", "").split("T")[0]
                    ...
                    if file.get("webViewLink"):
                        response += (
                            f"   [📂 Open in Google Drive]({file['webViewLink']})\n"
                        )
                    response += "\n"
    ...
    def create_new_document(self, title: str, content: str = "") -> str:
        ...
        try:
            ...
            if result.startswith("{"):
                ...
                response += (
                    f"Document ID: `{doc_info.get('documentId', 'Unknown')}`\n\n"
                )
                ...
                if content:
                    response += "The document has been created with your initial content. You can now edit it in Google Docs!"
                else:
                    response += "The document has been created and is ready for you to add content!"
        ...
        try:
            ...
            if result.startswith("{"):
                ...
                if data:
                    ...
                    response += "The spreadsheet has been created and is ready for you to add data!"
