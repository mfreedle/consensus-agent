...
class Tools:
    ...
    def get_oauth_authorization_url(self) -> str:
        ...
        try:
            ...
            return (
                ...
                f"Copy the authorization code from the URL and use the 'complete_oauth_setup' function."
            )
    ...
    """
    def create_google_keep_note_http(self, title: str, content: str = "") -> str:
        """
        Create a new Google Keep note using direct HTTP API calls.

        :param title: The title of the note.
        :param content: The content/body of the note (optional).
        :return: Status message indicating success or failure.
        """
        try:
            ...
            result = self._make_api_request(keep_url, "POST", note_data)
            
            if "error" in result:
                return f"❌ Error creating Keep note: {result['error']}"

            note_name = result.get("name", "Unknown")
            note_title = result.get("title", title)
            
            return f"✅ Google Keep note '{note_title}' created successfully! Note ID: {note_name}"

        except Exception as e:
            return f"❌ Error creating Google Keep note: {str(e)}"

    def list_google_keep_notes_http(self, max_results: int = 10) -> str:
        """
        List Google Keep notes using direct HTTP API calls.

        :param max_results: Maximum number of notes to return.
        :return: JSON string containing notes list.
        """
        try:
            keep_url = "https://keep.googleapis.com/v1/notes"
            params = {
                "pageSize": max_results,
                "filter": "trashed=false"  # Only show non-trashed notes
            }

            result = self._make_api_request(keep_url, "GET", params=params)
            
            if "error" in result:
                return f"❌ Error listing Keep notes: {result['error']}"

            notes = result.get("notes", [])
            
            if not notes:
                return "No Google Keep notes found."
            ...
            for note in notes:
                notes_data.append({
                    ...
                    "createTime": note.get("createTime", "Unknown"),
                    "updateTime": note.get("updateTime", "Unknown"),
            ...
            return f"❌ Error listing Google Keep notes: {str(e)}"

    def update_google_keep_note_http(self, note_name: str, new_title: Optional[str] = None, new_content: Optional[str] = None) -> str:
        """
        Update an existing Google Keep note using direct HTTP API calls.

        :param note_name: The name/ID of the note to update.
        :param new_title: New title for the note (optional).
        :param new_content: New content for the note (optional).
        :return: Status message indicating success or failure.
        """
        try:
            keep_url = f"https://keep.googleapis.com/v1/{note_name}"
            ...
            if not update_data:
                return "❌ No updates provided. Specify new_title or new_content."

            result = self._make_api_request(keep_url, "PATCH", update_data)
            
            if "error" in result:
                return f"❌ Error updating Keep note: {result['error']}"

            note_title = result.get("title", "Unknown")
            return f"✅ Google Keep note '{note_title}' updated successfully!"

        except Exception as e:
            return f"❌ Error updating Google Keep note: {str(e)}"

    def search_google_keep_notes_http(self, query: str, max_results: int = 10) -> str:
        """
        Search Google Keep notes using direct HTTP API calls.

        :param query: Search query for note titles and content.
        :param max_results: Maximum number of results to return.
        :return: JSON string containing search results.
        """
        try:
            ...
            params = {
                "pageSize": max_results,
                "filter": f"trashed=false AND (title:{query} OR text:{query})"
            }

            result = self._make_api_request(keep_url, "GET", params=params)
            
            if "error" in result:
                return f"❌ Error searching Keep notes: {result['error']}"
            ...
            for note in notes:
                notes_data.append({
                    "name": note.get("name", "Unknown"),
                    "title": note.get("title", "Untitled"),
                    "content": note.get("body", {}).get("text", {}).get("text", "")[:100] + "..." if len(note.get("body", {}).get("text", {}).get("text", "")) > 100 else note.get("body", {}).get("text", {}).get("text", ""),
                    "createTime": note.get("createTime", "Unknown"),
                    "updateTime": note.get("updateTime", "Unknown"),
                })
            ...
            return f"❌ Error searching Google Keep notes: {str(e)}"
    """
