...
async def seed_initial_data():
    ...
    try:
        ...
        if is_railway:
            logger.info(
                "Railway deployment detected - seeding models without force refresh to preserve existing data"
            )
...
async def seed_models(force_refresh=False):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if existing_models and not force_refresh:
            logger.info(
                f"Models already exist ({len(existing_models)} models). Skipping seeding. Use force_refresh=True to reseed."
            )
            return
        ...
        logger.info(
            f"Seeded {len(models)} comprehensive LLM models from LLM_models_to_use.md"
        )
...
async def create_default_user():
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            await session.commit()
            logger.info(
                f"Created default admin user: {admin_username} with password: {admin_password}"
            )
    ...
    try:
        ...
        async with engine.begin() as conn:
            if settings.database_url.startswith("postgresql"):
                ...
                if not table_exists:
                    logger.info(
                        "Users table doesn't exist yet, will be created by create_all"
                    )
                    return
