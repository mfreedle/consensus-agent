...
class GoogleDriveTools:
    ...
    async def execute_function(
        self, function_name: str, parameters: Dict[str, Any], user: User
    ...
    async def _list_files(
        ...
        return GoogleDriveToolResult(
            ...
            data={"files": files, "count": len(files), "file_type": file_type},
        )
    ...
    async def _read_document(
        ...
        document = await self.google_service.get_document_content(
            document_id=file_id, access_token=access_token, refresh_token=refresh_token
        )
    ...
    async def _create_document(
        ...
        return GoogleDriveToolResult(
            success=True, message=f"Successfully created document: {title}", data=result
        )
    ...
    async def _create_spreadsheet(
        ...
        result = await self.google_service.create_spreadsheet(
            access_token=access_token, title=title, refresh_token=refresh_token
        )
    ...
    async def _get_file_path(
        ...
        path = await self.google_service.get_file_path(
            file_id=file_id, access_token=access_token, refresh_token=refresh_token
        )
    ...
    async def _list_all_files_with_paths(
        self,
        parameters: Dict[str, Any],
        access_token: str,
        refresh_token: Optional[str],
